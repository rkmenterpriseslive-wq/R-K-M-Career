...
import { Job, UserType, JobListProps } from '../types';
import Button from './Button';

const JobList: React.FC<JobListProps> = ({ jobs, currentUserType, onDeleteJob, onApplyNow, onViewDetails }) => {
        ...
        <div key={job.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex flex-col justify-between relative">
          ...
          <div>
            <div className="flex justify-between items-start mb-2 pr-16"> {/* Add padding-right to avoid overlap with logo */}
              ...
              {/* Experience Level badge will be at the bottom row now */}
            </div>
            <p className="text-gray-600 text-sm font-medium">{job.partnerName || job.company}</p>
            <p className="text-gray-600 text-sm flex items-center mt-1">
              ...
              {job.jobCity}{job.locality && job.locality !== job.jobCity ? `, ${job.locality}` : ''} {job.storeName && job.storeName !== job.locality && `| ${job.storeName}`}
            </p>
          ...
          <div className="mt-4 flex items-center justify-between"> {/* Changed to justify-between to space out items */}
            {/* Experience Level Re-added here */}
            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
              job.experienceLevel === 'Fresher' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
            }`}>
              {job.experienceLevel}
            </span>
            <div className="flex gap-2"> {/* Group buttons together */}
                {onViewDetails && (
                    <Button variant="secondary" size="sm" onClick={() => onViewDetails(job)} className="min-w-[80px]">
                        More Info
                    </Button>
                )}
                {isAdmin && onDeleteJob ? (
                ...
                <Button variant="primary" size="md" onClick={() => onApplyNow?.(job)} className="min-w-[100px]">
                    Apply Now
                </Button>
                )}
            </div>
