...
const CandidateSidebar: React.FC<CandidateSidebarProps> = ({ activeItem, onItemClick, isProfileComplete, isOpen, onClose }) => {
  const menuItems = Object.values(CandidateMenuItem);

  const handleItemClick = (item: CandidateMenuItem) => {
    onItemClick(item);
    if (onClose) onClose();
  };
